...
router.post('/register', async (req, res, next) => {
  try {
    ...
    if (name.length < 2 || !/^\S+@\S+\.\S+$/.test(email) || password.length < 8) {
      return res.status(400).render('register', { title:'Create account', error:'Enter a valid name, email and password of at least 8 characters.' });
    }
    const exists = await pool.query('SELECT id FROM users WHERE email=$1', [email]);
    if (exists.rowCount) return res.status(409).render('register', { title:'Create account', error:'An account with that email already exists.' });
